...
int ProductsInput(ProductList* list, FILE* lombard);
...
void main(void)
{
    ...
    {
        printf("File is open\n");
        while (ProductsInput(List, lombard));
    }
...
void pushBack(BuyerInfo* list)
{
    ...
    scanf("%s", client->value->Passport);
    printf("Enter your date of registration: ");
    scanf("%s", client->value->date);
    printf("Have u ever been here b4? (Y or N) ");
    scanf("%s", client->value->reputation);
    printf("\n");
    client->next = NULL;
...
void PrintAdminInfo(BuyerInfo* Client, ProductList* List)
{
    ...
    while (client)
    {
        ...
        printf("Passport: %s\n\n", client->value->Passport);
        printf("Date: %s\n\n", client->value->date);
        printf("Reputation: %s\n\n", client->value->reputation);
...
void PrintProduct(ProductList* list, int id)
{
    ...
    while (Product)
    {
        ...
        if (comparison == id)
        {
            ...
            printf("Title: %s\n", Product->value->Name);
            printf("Price(BYN): %d\n", Product->value->price);
            printf("the price of bail: %d\n", Product->value->mainprice);
...
int GetPrice(ProductList* list, int number)
{
    ...
    return Product->value->mainprice;
}
...
int ProductsInput(ProductList* list, FILE* lombard)
{
    ...
    catalog->value->mainprice = 0;
    if (fscanf(lombard, "%d%d%s%d%d%s%s", &catalog->value->identifier, &catalog->value->number, catalog->value->Name,
        &catalog->value->price, &catalog->value->mainprice, catalog->value->guarantee, catalog->value->country) != EOF)
    {
        catalog->next = NULL;
...
void MinusAmount(ProductList* List, BuyerInfo* Client, int current)
{
    ...
    Product->value->mainprice = Product->value->mainprice - 1;
}
...
void ShowBag(ProductList* list, BuyerInfo* Client, int SizeBag)
{
    ...
    if (SizeBag > 0)
    {
        ...
        for (int i = 0; i < SizeBag; i++)
        {
            bill += GetPrice(list, client->value->Bag[i])*0.2;
        }
        printf("%d \n", bill);
        printf("1)Buy\n");
